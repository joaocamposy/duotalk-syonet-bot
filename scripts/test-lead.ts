...
const serviceUrl = process.env.API_URL ?? 'http://127.0.0.1:3000';
const serviceToken = process.env.API_TOKEN;
const requestTimeoutMs = Number(process.env.API_REQUEST_TIMEOUT_MS ?? 75_000);
...
if (!Number.isFinite(requestTimeoutMs) || requestTimeoutMs <= 0) {
  throw new Error('API_REQUEST_TIMEOUT_MS deve ser um número positivo');
}
...
const response = await fetch(`${serviceUrl.replace(/\/$/, '')}/leads`, {
  method: 'POST',
  headers: {
